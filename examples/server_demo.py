...
def check_server_ready(
    host: str, port: int, max_retries: int = 10, retry_delay: float = 1.0
) -> bool:
    """
...
def make_chat_request(
    host: str, port: int, model: str, messages: List[Dict[str, str]]
) -> Dict[str, Any]:
    """
...
def demo_openai_client(host: str, port: int, model: str) -> None:
    ...
    client = OpenAI(
        api_key="REDACTED", base_url=f"http://{host}:{port}/v1"  # Not used but required
    )
...
def main():
    ...
    parser.add_argument(
        "--start-server", action="store_true", help="Start server in a subprocess"
    )
