...
from .backends import BACKENDS, BaseBackend, get_backend
...
class LlamaForge:
    ...
    def __init__(
        ...
        if model_name or self.config.get("default_model"):
            self.load_model(
                model_name or self.config.get("default_model"), backend_name
            )
        ...
        for plugin_config in plugins_config:
            ...
            try:
                if plugin_type == "preprocessing" and plugin_name in PREPROCESSORS:
                    ...
                    logger.warning(
                        f"Unknown plugin type or name: {plugin_type}/{plugin_name}"
                    )
    ...
    def load_model(self, model_name: str, backend_name: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not model_info and not os.path.exists(model_name):
                logger.error(
                    f"Model '{model_name}' not found in configuration or file system"
                )
                return False
            ...
            # Create model
            model = Model(name=model_name, path=model_path, config=model_info)
        ...
        for postprocessor in self.postprocessors:
            if getattr(postprocessor, "supports_streaming", False):
                try:
                    ...
                    logger.error(
                        f"Error in streaming postprocessor {postprocessor.name}: {str(e)}"
                    )
            ...
            if os.path.isdir(model_dir):
                for file in os.listdir(model_dir):
                    ...
                    if os.path.isfile(path) and any(
                        path.endswith(ext) for ext in [".bin", ".gguf", ".ggml"]
                    ):
