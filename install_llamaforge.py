...
def check_python_version():
    ...
    if sys.version_info < (3, 8):
        logger.error(
            "Python 3.8 or higher is required. Current version: {}.{}".format(
        ...
        for backend in backends:
            ...
            if deps:
                ...
                try:
                    ...
                    logger.warning(
                        f"Installation for backend {backend} failed: {e}. Continuing installation."
                    )
...
def get_source_file(file_name: str, current_dir: Path):
    ...
    logger.warning(
        f"Source file {file_name} not found. Installation may be incomplete."
    )
    return None
...
def add_to_system_path(install_dir: Path, launcher_file: Path):
    ...
    if system in ["Linux", "Darwin"]:  # Unix-like systems
        ...
        try:
            ...
            logger.info(
                f"To add LlamaForge to PATH, run: sudo ln -s {launcher_file} {symlink_path}"
            )
        ...
        try:
            ...
            logger.info(
                f"To add LlamaForge to PATH, add {install_dir} to your PATH environment variable."
            )
...
def main():
    ...
    parser.add_argument(
        "--dir", help="Installation directory", default=str(DEFAULT_INSTALL_DIR)
    )
    parser.add_argument(
        "--backends",
        help="Comma-separated list of backends to install (mlx,llama.cpp,transformers,all)",
        default="all",
    )
    parser.add_argument(
        "--no-path", help="Do not add LlamaForge to system PATH", action="store_true"
    )
    parser.add_argument(
        "--no-sample-data", help="Do not download sample data", action="store_true"
    )
    ...
    if args.backends.lower() == "all":
        ...
        if "mlx" in backends and not platform_info["mlx_supported"]:
            logger.warning(
                "MLX backend is not supported on this platform. Skipping MLX installation."
            )
            backends.remove("mlx")
    ...
    print(
        "  llamaforge --download TheBloke/Mistral-7B-Instruct-v0.1-GGUF  # Download a model"
    )
